...
import { setupTestSuite, AMOUNT, deployTokenWithMinter, initializeTransferCommitment } from './utils';
...
describe('Token', () => {
  ...
  it('mint in public, prepare partial note and finalize it', async () => {
    ...
    const bobAccountManager = await wallet.createSchnorrAccount(accountData.secret, accountData.salt, accountData.signingKey);
  ...
  it('private transfer with authwitness', async () => {
    ...
    const intent = { caller: carl, call: await action.getFunctionCall() };
